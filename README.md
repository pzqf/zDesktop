...
1. 托盘右键 → **分区编辑模式**
2. 在桌面空白处拖出一个矩形 → 分区创建完成
3. 再次点击「分区编辑模式」退出 —— 此时桌面图标恢复正常点击
4. 把图标拖进分区范围即归属该分区，拖出即解除
5. 托盘 → 分区 → **一键整理** 按规则批量归位（执行前会预览并落盘快照，可随时撤销）

分区主体不拦鼠标，里面的图标仍是原生图标：双击打开、F2 改名、框选、右键菜单全都照常。
...
| 桌面分区 | 可用（0.1）：拖拽新建、移动/缩放/折叠/重命名、归属自动同步、一键整理 + 撤销、背景合成进壁纸 |
