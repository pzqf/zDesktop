...
| 覆盖层 / Z 序控制 | 可用；Z 序自愈与工作区 DPI 换算已完成，多屏与 PerMonitorV2 待 M1 |
| 桌面分区 | **未实现**（v3.0 主线，技术方案待 spike 验证） |
| 壁纸 | 仅静态壁纸 + 必应每日，无动态壁纸引擎 |
| 任务栏增强 / 图标着色 / 多窗格文件管理 | 已从范围内移除，见设计案 v3.0 §3.3 |

**遗留限制**：多显示器下覆盖层仅覆盖主屏工作区，副屏无组件；进程尚未声明 `PerMonitorV2`，跨不同缩放的显示器移动时组件不会重新适配。两项均排在 M1。

「zDesktop 图标」（托盘菜单，默认关闭）是会隐藏原生图标层的实验特性，开启后回收站/此电脑/副屏图标将不可见，且缺少框选、F2 改名等原生能力——不建议开启。
